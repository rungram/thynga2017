...
if($_GET['id'])
{
	...
	$d->reset();
	$sql ="select id_order from #_order where id ='".$_GET['id']."'";
	$d->query($sql);	
	$result =$d->fetch_array(); 
	
	$_SESSION['cart'] = array();
	$d->reset();
	$sql ="select * from #_donhang where id_order ='".$result['id_order']."'	order by id desc";
	$d->query($sql);	
	$list_donhang =$d->result_array();	
	if(count($list_donhang))
	for($i=0,$ct=count($list_donhang);$i<$ct;$i++){
	array_push($_SESSION['cart'],array('productid' => $list_donhang[$i]['id_sp'],'qty' => $list_donhang[$i]['soluong']));
	}
...
<div class="container-fluid">
	...
	<form name="formgiobo" method="post">
		...
		<tr bgcolor="#FFFFFF">
			...
			<td width="29%"><a href="chi-tiet-san-pham/<?=$pkodau?>-<?=$pid?>.html">
			...
			<td width="15%" align="center"><select onChange="update_cart()" name="product<?=$pid?>" >
				<?php
			for($k=1;$k<=20;$k++)
			{
			?>
				<option value="<?=$k?>" <?php	if($q==$k) echo 'selected="selected"';	 ?>>
				<?=$k?>
				</option>
				<?php
			}
		...
		?>
		<tr>
			<td colspan="8"><i style="padding:10px">Đặt hàng qua điện thoại
			<?=$row_setting['hotline']?>
			</i>
			<div class="right_gh">
				<ul>
				<li>Phí vận chuyển <span>
					<?= number_format($row_setting['phivc'],0, ',', '.').'đ';?>
					</span></li>
		...
		<tr>
			<td colspan="6" align="right"><div class="canhnut">
				<input type="button" value="Mua tiếp" onClick="window.location='http://<?=$config_url?>/index.html'" class="nutgh">
				<input type="button" value="Gửi đơn hàng" onClick="window.location='http://<?=$config_url?>/thanh-toan.html'" class="nutgh">
			</div></td>
		</tr>
		<?php
		}
