...
<script language="javascript">			
	...
	{		
		...
		window.location ="index.php?com=product3&act=<?php if($_REQUEST['act']=='edit') echo 'edit'; else echo 'add';?><?php if($_REQUEST['id']!='') echo"&id=".$_REQUEST['id']; ?>&id_list="+b.value;	
		return true;
	}
...
function get_main_list()
	{
		$sql="select * from table_product3_list order by stt,id asc";
	...
	{
		$sql_huyen="select * from table_product3_cat where id_list=".$_REQUEST['id_list']." order by id asc ";
...
<form name="frm" method="post" action="index.php?com=product3&act=save&curPage=<?=$_REQUEST['curPage']?>" enctype="multipart/form-data" class="nhaplieu">
    <b>Danh mục cấp 1:</b><?=get_main_list();?><br /><br />
	<b>Danh mục cấp 2:</b><?=get_main_cat();?><br /><br />
    <b>Danh mục cấp 3:</b><?=get_main_loai();?><br /><br />
	<?php if ($_REQUEST['act']==edit)
	{?>
	<b>Hình hiện tại:</b><img src="<?=_upload_sanpham3.$item['thumb']?>"  width="120" alt="NO PHOTO" /><br />
	<?php }?>
	<b>Hình ảnh:</b> <input type="file" name="file" /> <?=_product3_type?><br />
    <br />
    <b>Tên sản phẩm việt</b> <input type="text" name="ten_vi" value="<?=$item['ten_vi']?>" class="input" /><br />  
    <b>Tên sản phẩm anh</b> <input type="text" name="ten_en" value="<?=$item['ten_en']?>" class="input" /><br />   
    <b>Tên sản phẩm trung</b> <input type="text" name="ten_cn" value="<?=$item['ten_cn']?>" class="input" /><br />     
    <b>Số lượng</b> <input type="text" name="soluong" value="<?=$item['soluong']?>" class="input" /><br />
    <b>Bảo hành</b> <input type="text" name="baohanh" value="<?=$item['baohanh']?>" class="input" /><br />     
    <b>Trình trạng</b> <input type="text" name="trinhtrang" value="<?=$item['trinhtrang']?>" class="input" /><br />     
    <b>Xuất xứ</b> <input type="text" name="xuatxu" value="<?=$item['xuatxu']?>" class="input" /><br />     
    <b>Vận chuyển</b> <input type="text" name="vanchuyen" value="<?=$item['vanchuyen']?>" class="input" /><br />      
	...
	<input type="button" value="Thoát" onclick="javascript:window.location='index.php?com=product3&act=man'" class="btn" />
</form>