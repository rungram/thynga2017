...
<script language="javascript">				
	...
	{				
		...
		window.location ="index.php?com=product2&act=<?php if($_REQUEST['act']=='edit_item') echo 'edit_item'; else echo 'add_item';?><?php if($_REQUEST['id']!='') echo"&id=".$_REQUEST['id']; ?>&id_list="+a.value;	
		return true;
	}
...
function get_main_list()
	{
		$sql="select * from table_product2_list order by stt";
	...
	{
		$sql_huyen="select * from table_product2_cat where id_list=".$_REQUEST['id_list']." order by id desc ";
...
?>
<form name="frm" method="post" action="index.php?com=product2&act=save_item&curPage=<?=$_REQUEST['curPage']?>" enctype="multipart/form-data" class="nhaplieu">	 
	...
	<input type="button" value="Thoát" onclick="javascript:window.location='index.php?com=product2&act=man_item'" class="btn" />
</form>